...
def calculate_sustainability_ratings(file_path):
    ...
    numeric_columns = df_sustainability.columns.drop(
        ['Property Id', 'Latitude', 'Longitude'])
    df_sustainability[numeric_columns] = df_sustainability[numeric_columns].apply(
    ...
    df_sustainability.drop(
        columns=['Property Id', 'Latitude', 'Longitude'], inplace=True)

    # Apply MinMaxScaler
    scaler = MinMaxScaler()
    df_sustainability_scaled = scaler.fit_transform(df_sustainability)
    df_sustainability = pd.DataFrame(
        df_sustainability_scaled, columns=df_sustainability.columns)

    # Calculate the composite sustainability score
    df_sustainability['Composite Sustainability Score'] = df_sustainability.mean(
        axis=1)
...
def get_coordinates(api_key, location):
    ...
    try:
        response = requests.get(
            base_url, params={"address": location, "key": api_key})
...
def find_nearest_locations(api_key, location, df, k):
    ...
    df_sorted = df.sort_values(
        by=['Distance', 'Normalized Sustainability Score'], ascending=[True, False])
